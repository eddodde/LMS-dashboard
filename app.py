...
with tab1:
    ...
    # ── 채널별 성과 비교 (연도 2개↑: 전년비 / 1개: 월별) ──────────────────────────────────────────────
    num_years = df_with_perf['연도'].nunique()
    cmp_metric = st.radio("비교 지표", ['평균CTR', '평균CR', '평균ROAS', '총거래액', '발송건수'], horizontal=True, key='yoy_m')
    ...
    fig_cmp = px.line(
        ...
        labels={x_col: x_label, cmp_metric: cmp_metric}
    )
    ...
    # 인사이트
    if num_years >= 2:
        years = sorted(plot_df['연도'].unique())
        prev_y, curr_y = years[-2], years[-1]
        ins = []
        for ch in sorted(plot_df['채널'].unique()):
            p = plot_df[(plot_df['연도'] == prev_y) & (plot_df['채널'] == ch)][cmp_metric]
            c = plot_df[(plot_df['연도'] == curr_y) & (plot_df['채널'] == ch)][cmp_metric]
            if len(p) and len(c):
                diff = c.values[0] - p.values[0]
                ins.append(f"**{ch}**: {prev_y}→{curr_y} {'↑' if diff>0 else '↓'} {fmt_val(abs(diff), cmp_metric)} 변동")
        if ins:
            st.info("  \n".join(ins))
    else:
        best = plot_df.loc[plot_df[cmp_metric].idxmax()]
        st.info(f"{cmp_metric} 최고 월: **{best[x_col]}** — {best['채널']} {fmt_val(best[cmp_metric], cmp_metric)}")
...
with tab4:
    ...
    if recurring_groups:
        ...
        with st.expander(f"'{selected_group}' {label} 상세"):
            st.dataframe(rec_agg.style.format({
